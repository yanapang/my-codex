...
- Сеансы вне team теперь работают по модели native-hook-first.
- `omx tmux-hook` теперь предназначен для team runtime и legacy-отладки tmux.
- Файлы плагинов располагаются в `.omx/hooks/*.mjs`.
- Плагины по умолчанию включены; отключайте через `OMX_HOOK_PLUGINS=0`.
