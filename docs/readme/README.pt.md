...
- Sessões fora de team agora são native-hook-first.
- `omx tmux-hook` fica reservado para runtime de team e troubleshooting tmux legado.
- Arquivos de plugins ficam em `.omx/hooks/*.mjs`.
- Plugins ficam ativados por padrão; desative com `OMX_HOOK_PLUGINS=0`.
