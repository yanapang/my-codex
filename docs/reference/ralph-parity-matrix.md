...
| R5 | Current session scope is authoritative when active session exists; same-Codex-session continuation may migrate a single active Ralph into that scope. Root is compatibility fallback. | `src/mcp/state-paths.ts`, `src/hud/state.ts`, `src/cli/index.ts`, `scripts/notify-hook.js` | adapted | V1, V2, V8 |
