...
|---|---|---|---|
| V1 | Session-scoped Ralph lifecycle + same-session ownership transfer | `src/cli/__tests__/session-scoped-runtime.test.ts` + `src/mcp/__tests__/trace-server.test.ts` + `src/hooks/__tests__/notify-hook-ralph-resume.test.ts` + `src/hooks/__tests__/notify-hook-session-scope.test.ts` | [x] |
...
| V8 | Cross-session safety + single-owner migration | `src/cli/__tests__/session-scoped-runtime.test.ts` + `src/mcp/__tests__/trace-server.test.ts` + `src/hooks/__tests__/notify-hook-ralph-resume.test.ts` + `src/hooks/__tests__/notify-hook-session-scope.test.ts` | [x] |
