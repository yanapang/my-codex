...
- In user scope, `omx setup` can keep using the resolved user skill root (`legacy`) or rely on plugin discovery (`plugin`). Plugin mode only removes matching OMX-managed legacy user skill directories after user-visible plugin cache readiness is proven.
